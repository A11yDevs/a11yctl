...
function Invoke-SelfUpdate {
    ...
    $files = @('a11yctl.ps1', 'a11yctl.cmd', 'ea11ctl.ps1', 'ea11ctl.cmd', 'install.ps1', 'VERSION')
...
function Invoke-StateMigration {
    ...
    foreach ($directory in $directories) {
        $relativePath = $directory.FullName.Substring($legacyDir.Length).TrimStart('\\', '/')
    ...
    foreach ($file in $files) {
        $relativePath = $file.FullName.Substring($legacyDir.Length).TrimStart('\\', '/')
...
function Invoke-MigrateStateCommand {
    param([string[]]$Tokens)

    $quiet = Has-Flag -Tokens $Tokens -Flags @('--quiet')
    [void](Invoke-StateMigration -Quiet:$quiet)
}
...
function Invoke-Uninstall {
    ...
    $toRemove = @('a11yctl.cmd', 'ea11ctl.cmd', 'ea11ctl.ps1', 'install.ps1', 'VERSION')
...
function Invoke-RootCommand {
    ...
    switch ($root) {
        ...
        'migrate' { Invoke-MigrateStateCommand -Tokens $rest }
        'migrate-state' { Invoke-MigrateStateCommand -Tokens $rest }
