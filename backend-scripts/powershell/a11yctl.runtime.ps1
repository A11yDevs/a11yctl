...
$EA11CTL_FALLBACK_VERSION = '0.2.0'
...
function Show-Help {
    @"
a11yctl - CLI do projeto a11yctl [HOST - WINDOWS]
...
Uso:
  ...
  a11yctl vm autologin on|off|status [-n|--name VM] [-u|--user USER] [-p|--port PORT]

Nota: Dentro da VM (guest context), execute: a11yctl share
Compatibilidade temporaria: ea11ctl continua disponivel como alias com aviso de depreciacao.
...
function Get-GitHubApiHeaders {
    return @{
        'User-Agent' = "a11yctl/$($EA11CTL_FALLBACK_VERSION)"
...
function Invoke-AccessibleDownload {
    ...
    $request.Method = 'GET'
    $request.UserAgent = "a11yctl/$($EA11CTL_FALLBACK_VERSION)"
...
function Get-LegacyEA11StateDirectory {
    return (Join-Path (Get-HomeDirectoryPath) $A11YCTL_LEGACY_STATE_DIRNAME)
}
...
function Resolve-MigrationConflictPath {
    param([string]$Path)
    ...
    if ([string]::IsNullOrWhiteSpace($extension)) {
        $candidate = Join-Path $directory ($baseName + '.migrated')
    }
    else {
        $candidate = Join-Path $directory ($baseName + '.migrated' + $extension)
    }

    $counter = 2
    while (Test-Path $candidate) {
        if ([string]::IsNullOrWhiteSpace($extension)) {
            $candidate = Join-Path $directory ("{0}.migrated-{1}" -f $baseName, $counter)
        }
        else {
            $candidate = Join-Path $directory ("{0}.migrated-{1}{2}" -f $baseName, $counter, $extension)
        }
        $counter++
    }
...
function Invoke-StateMigration {
    ...
    $legacyDir = Get-LegacyEA11StateDirectory
    if (-not (Test-Path $legacyDir)) {
        if (-not $Quiet) {
            Write-EA11Info "Nenhum diretorio legado encontrado em $legacyDir"
        }
        return $false
    }
    ...
    foreach ($directory in $directories) {
        ...
        $destinationDirectory = Join-Path $targetDir $relativePath
    ...
    foreach ($file in $files) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($destinationDirectory) -and (-not (Test-Path $destinationDirectory))) {
            New-Item -ItemType Directory -Path $destinationDirectory -Force | Out-Null
        }

        if (Test-Path $destination) {
            $destination = Resolve-MigrationConflictPath -Path $destination
            $renamedCount++
        }
    ...
    if (-not $Quiet) {
        ...
        Write-EA11Info "O diretorio legado foi preservado em $legacyDir"
        Write-EA11Info "Novo diretorio de estado: $targetDir"
    }
...
function Save-QemuRuntimeConfig {
    ...
    $cfgDir = [System.IO.Path]::GetDirectoryName($cfgPath)
    if (-not (Test-Path $cfgDir)) { New-Item -ItemType Directory -Path $cfgDir -Force | Out-Null }
    $lines = @('# Configuracao de runtime do QEMU para a11yctl', '# Edite com cuidado. Valores invalidos podem impedir o boot.')
        ...
        'cpus' {
            if ($Value -match '^[1-9][0-9]*$') {
                ...
                if (($null -ne $limit) -and ([int]$Value -gt [int]$limit.Max)) {
                    ...
                    Write-Host ''
                    Write-Host 'Exemplos:'
                    Write-Host '  a11yctl vm config set cpus 2'
                    Write-Host '  a11yctl vm config set cpus 4'
                    return $null
                }
                return $Value
            }
            ...
            Write-Host '  a11yctl vm config set cpus 2'
            Write-Host '  a11yctl vm config set cpus 4'
            Write-Host '  a11yctl vm config set cpus 8'
            return $null
        }
        'memory' {
            if ($Value -match '^[1-9][0-9]*$') {
                ...
                if (($null -ne $limit) -and ([int]$Value -gt [int]$limit.Max)) {
                    ...
                    Write-Host ''
                    Write-Host 'Exemplos:'
                    Write-Host '  a11yctl vm config set memory 2048'
                    Write-Host '  a11yctl vm config set memory 4096'
                    return $null
                }
                return $Value
            }
            ...
            Write-Host '  a11yctl vm config set memory 2048'
            Write-Host '  a11yctl vm config set memory 4096'
            Write-Host '  a11yctl vm config set memory 8192'
            return $null
        }
        'fullscreen' {
            ...
            Write-Host ''
            Write-Host 'Exemplos:'
            Write-Host '  a11yctl vm config set fullscreen on'
            Write-Host '  a11yctl vm config set fullscreen off'
            return $null
        }
        'accel' {
...
function Show-QemuConfigGet {
    ...
    if ($null -eq $internalKey) {
        ...
        Write-Host ''
        Write-Host 'Use:'
        Write-Host '  a11yctl vm config list'
        Write-Host ''
        Write-Host 'Exemplos:'
        Write-Host '  a11yctl vm config get memory'
        Write-Host '  a11yctl vm config get cpus'
        Write-Host '  a11yctl vm config get fullscreen'
        return $false
    }
    $cfg = Get-QemuRuntimeConfig
    $currentVal = $cfg[$internalKey]
...
function Invoke-QemuConfigSet {
    ...
    foreach ($pair in $Pairs) {
        ...
        if ($eqIdx -lt 0) {
            ...
            Write-Host 'Exemplo: a11yctl vm config set memory=8192 cpus=4 fullscreen=off'
            return $false
        }
...
function Show-QemuConfigHelp {
    Write-Host 'a11yctl vm config - Gerenciar configurações da VM'
    Write-Host ''
    Write-Host 'Uso:'
    Write-Host '  a11yctl vm config                          Mostra configuração amigável'
    Write-Host '  a11yctl vm config --raw                    Mostra variáveis técnicas (QEMU_*)'
    Write-Host '  a11yctl vm config list                     Lista todas as chaves configuráveis'
    Write-Host '  a11yctl vm config get CHAVE [--raw]        Consulta um valor'
    Write-Host '  a11yctl vm config set CHAVE VALOR          Define um valor'
    Write-Host '  a11yctl vm config set CHAVE=VALOR [...]    Define um ou mais valores'
    Write-Host '  a11yctl vm config path                     Mostra caminho do arquivo de config'
    Write-Host '  a11yctl vm config reset                    Reseta para valores padrão'
    Write-Host '  a11yctl vm config help                     Mostra esta ajuda'
    Write-Host ''
    ...
    Write-Host '  a11yctl vm config set cpus 4'
    Write-Host '  a11yctl vm config set fullscreen off'
    Write-Host '  a11yctl vm config set memory=8192 cpus=4 fullscreen=off'
    Write-Host '  a11yctl vm config get memory'
    Write-Host '  a11yctl vm config get memory --raw'
}
    ...
    switch ($action) {
        ...
        'get' {
            if ($Tokens.Length -lt 2) {
                Write-Host 'Erro: informe a chave. Exemplo: a11yctl vm config get memory' -ForegroundColor Red
                return
            }
        ...
        'set' {
            if ($Tokens.Length -lt 2) {
                Write-Host 'Erro: use: a11yctl vm config set CHAVE VALOR' -ForegroundColor Red
                Write-Host 'Ou:   a11yctl vm config set CHAVE=VALOR [CHAVE=VALOR ...]'
                return
            }
        ...
        default {
            throw "Ação de config desconhecida: $action. Use: a11yctl vm config help"
        }
    }
}
...
function Invoke-QemuVMOptimize {
    ...
    Write-EA11Info 'Use: a11yctl vm config show'
    Write-EA11Info 'Se houver regressao, restaure o backup ou execute: a11yctl vm config reset'
}
...
function Write-QemuArgsLog {
    ...
    $lines = @('# QEMU arguments for a11yctl', $QemuExecutable)
    $lines += $QemuArgs
...
function Install-EA11VMShortcut {
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($scriptPath)) {
            $scriptPath = Join-Path $PSScriptRoot 'a11yctl.ps1'
        }

        if (-not (Test-Path $scriptPath)) {
            Write-EA11Warn "Nao foi possivel localizar o script a11yctl.ps1 para criar o atalho."
            return
        }
...
function Invoke-VMInstall {
    ...
    if ((Test-Path $targetDisk) -and (-not $forceDownload)) {
        ...
        Write-EA11Info "Auto-login: padrao ON. Controle com: a11yctl vm autologin on|off|status"
    ...
    Save-QemuState -VMName 'debian-a11y' -State @{
        ...
        autoLogin = if ($existingState -and $existingState.autoLogin) { $existingState.autoLogin } else { 'on' }
    }
...
function Invoke-QemuVMStart {
    ...
    Save-QemuState -VMName $vmName -State @{
        ...
        lastStatus = 'running'
        autoLogin = if ($existing -and $existing.autoLogin) { $existing.autoLogin } else { 'on' }
    }
    ...
    Apply-EA11VMAutoLoginAfterStart -VMName $vmName -User 'a11ydevs' -Port $sshPort
}
...
function Invoke-QemuVMStop {
    ...
    if (-not $proc) {
        ...
        Save-QemuState -VMName $vmName -State @{
            ...
            lastStatus = 'stopped'
            autoLogin = if ($state -and $state.autoLogin) { $state.autoLogin } else { 'on' }
        }
        return
    }
...
function Invoke-QemuVMSSH {
    ...
    $hostKeyAlias = "a11yctl-$vmName-$port"
    ...
    $knownHostsFile = Join-Path $sshDir 'known_hosts_a11yctl'
...
function Invoke-EA11GuestCommand {
    param(
        [string]$VMName = 'debian-a11y',
        [string]$User = 'a11ydevs',
        [int]$Port = 2222,
        [string]$Command
    )
    ...
    if ([string]::IsNullOrWhiteSpace($User) -or ($User -notmatch '^[a-z_][a-z0-9_-]*[$]?$')) {
        throw "Usuario invalido para auto-login: '$User'"
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "SSH retornou codigo $LASTEXITCODE ao executar comando na VM."
    }
}

function Set-EA11VMAutoLoginState {
    param(
        [string]$VMName = 'debian-a11y',
        [string]$Mode = 'on'
    )
    ...
    Save-QemuState -VMName $VMName -State @{
        name = if ($state.name) { $state.name } else { $VMName }
        backend = if ($state.backend) { $state.backend } else { 'qemu' }
        pid = if ($state.pid) { $state.pid } else { $null }
        sshPort = if ($state.sshPort) { $state.sshPort } else { 2222 }
        sshUser = if ($state.sshUser) { $state.sshUser } else { 'a11ydevs' }
        systemDisk = if ($state.systemDisk) { $state.systemDisk } else { $null }
        userDataDisk = if ($state.userDataDisk) { $state.userDataDisk } else { $null }
        imageTag = if ($state.imageTag) { $state.imageTag } else { $null }
        homeMount = if ($state.homeMount) { $state.homeMount } else { '/home' }
        stdoutLog = if ($state.stdoutLog) { $state.stdoutLog } else { $null }
        stderrLog = if ($state.stderrLog) { $state.stderrLog } else { $null }
        qemuArgsLog = if ($state.qemuArgsLog) { $state.qemuArgsLog } else { $null }
        hostHomeSharePath = if ($state.hostHomeSharePath) { $state.hostHomeSharePath } else { $null }
        hostHomeShareTag = if ($state.hostHomeShareTag) { $state.hostHomeShareTag } else { $null }
        hostHomeShareMode = if ($state.hostHomeShareMode) { $state.hostHomeShareMode } else { $null }
        hostHomeSmbServer = if ($state.hostHomeSmbServer) { $state.hostHomeSmbServer } else { $null }
        hostHomeSmbShare = if ($state.hostHomeSmbShare) { $state.hostHomeSmbShare } else { $null }
        hostHomeGuestMountPoint = if ($state.hostHomeGuestMountPoint) { $state.hostHomeGuestMountPoint } else { $null }
        startedAt = if ($state.startedAt) { $state.startedAt } else { $null }
        stoppedAt = if ($state.stoppedAt) { $state.stoppedAt } else { $null }
        lastStatus = if ($state.lastStatus) { $state.lastStatus } else { 'stopped' }
        autoLogin = $Mode
    }
}

function Invoke-QemuVMAutoLogin {
    param([string[]]$Tokens)

    $mode = 'status'
    if ($Tokens -and $Tokens.Length -gt 0 -and ($Tokens[0] -notmatch '^-')) {
        $mode = ([string]$Tokens[0]).ToLowerInvariant()
    }

    if ($mode -notin @('on', 'off', 'status')) {
        throw "Uso: a11yctl vm autologin on|off|status [-n|--name VM] [-u|--user USER] [-p|--port PORT]"
    }

    $quiet = Has-Flag -Tokens $Tokens -Flags @('--quiet')
    $vmName = Get-VMName -Tokens $Tokens
    $state = Load-QemuState -VMName $vmName

    $userDefault = 'a11ydevs'
    if ($state -and $state.sshUser) {
        $userDefault = [string]$state.sshUser
    }

    $portDefault = 2222
    if ($state -and $state.sshPort) {
        $portDefault = [int]$state.sshPort
    }

    $user = Get-OptionValue -Tokens $Tokens -Names @('--user', '-u') -Default $userDefault
    $port = Get-IntOptionValue -Tokens $Tokens -Names @('--port', '-p') -Default $portDefault -OptionName '--port'
    
    # Define por padrão o autologin da VM = ON
    if ($mode -eq 'status') {
        $desired = 'on'
        if ($state -and $state.autoLogin) {
            $desired = [string]$state.autoLogin
        }

        Write-Host "Auto-login desejado: $desired"
        Write-Host "VM: $vmName"
        Write-Host "SSH: $user@localhost:$port"
        Write-Host "Comandos:"
        Write-Host "  a11yctl vm autologin on"
        Write-Host "  a11yctl vm autologin off"
        return
    }

    Set-EA11VMAutoLoginState -VMName $vmName -Mode $mode
...
printf '%s\n' '[Service]' 'ExecStart=' 'ExecStart=-/sbin/agetty --autologin $user --noclear %I `$TERM' | sudo tee /etc/systemd/system/getty@tty1.service.d/autologin.conf >/dev/null
...
"@
    ...
    try {
        Invoke-EA11GuestCommand -VMName $vmName -User $user -Port $port -Command $command
        if (-not $quiet) {
            if ($mode -eq 'on') {
                Write-EA11Info "Auto-login ligado para '$user'."
            }
            else {
                Write-EA11Info "Auto-login desligado."
            }
        }
    }
    catch {
        if ($mode -eq 'on') {
            Write-EA11Warn "Auto-login ficou marcado como ON, mas nao foi aplicado agora via SSH: $($_.Exception.Message)"
            Write-EA11Warn "Inicie a VM e execute: a11yctl vm autologin on"
        }
        else {
            Write-EA11Warn "Auto-login ficou marcado como OFF, mas nao foi removido agora via SSH: $($_.Exception.Message)"
            Write-EA11Warn "Com a VM ligada, execute: a11yctl vm autologin off"
        }
    }
}

function Apply-EA11VMAutoLoginAfterStart {
    param(
        [string]$VMName = 'debian-a11y',
        [string]$User = 'a11ydevs',
        [int]$Port = 2222
    ...
    $mode = 'on'
    if ($state -and $state.autoLogin) {
        $mode = [string]$state.autoLogin
    }

    if ($mode -eq 'off') {
        Write-EA11Info "Auto-login: OFF. Tentando garantir que a VM esteja sem auto-login..."
        if (Wait-EA11TcpPort -HostName 'localhost' -Port $Port -TimeoutSeconds 45) {
            Invoke-QemuVMAutoLogin -Tokens @('off', '--name', $VMName, '--user', $User, '--port', [string]$Port, '--quiet')
        }
        else {
            Write-EA11Warn "SSH ainda nao respondeu. Quando a VM terminar de subir, aplique com: a11yctl vm autologin off"
        }
        return
    }

    Write-EA11Info "Auto-login: ON por padrao. Tentando aplicar na VM via SSH..."
    if (Wait-EA11TcpPort -HostName 'localhost' -Port $Port -TimeoutSeconds 45) {
        Invoke-QemuVMAutoLogin -Tokens @('on', '--name', $VMName, '--user', $User, '--port', [string]$Port, '--quiet')
    }
    else {
        Write-EA11Warn "SSH ainda nao respondeu. Quando a VM terminar de subir, aplique com: a11yctl vm autologin on"
    }
}
...
function Invoke-VMShareFolder {
    ...
    if ($Tokens.Length -eq 0) {
        throw "Uso: a11yctl vm share-folder <add|remove|list> [opcoes]"
    }
...
function Invoke-VMHostShare {
    ...
    switch ($action) {
        ...
        default {
            throw "Acao desconhecida de host-share: $action. Use: a11yctl vm host-share list"
        }
    }
}
...
No Windows, use apenas:
  a11yctl vm install (baixa a imagem VM Debian pre-configurada)
  a11yctl vm start    (inicia a VM com Emacs + espeakup)
...
function Invoke-HostCommand {
    ...
    if ($Tokens.Length -eq 0) {
        throw "Uso: a11yctl host <install>"
    }
...
function Invoke-VMCommand {
    ...
    if ($cleanTokens.Length -eq 0) {
        throw "Uso: a11yctl vm <install|list|start|stop|close|remove|config|optimize|diagnose|status|ssh>"
    }
    ...
    switch ($sub) {
        ...
        'autologin' {
            Invoke-QemuVMAutoLogin -Tokens $rest
        }
        { $_ -in @('share-folder', '-F') } {
...
ssh            conecta via SSH
autologin      liga/desliga o auto-login da VM (on|off|status)
...
function Confirm-SensitiveCommand {
    ...
    Write-Host ('a11yctl ' + ($Command -join ' '))
    Write-Host ''
...
function Start-InteractiveShell {
    Write-Host 'a11yctl - modo interativo'
    Write-Host ''
